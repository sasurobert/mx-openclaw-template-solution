...
echo "🤖 mx-openclaw-template-solution — Setup Wizard"
echo "================================================"
echo ""

# 1. Bot Name
read -p "📛 Bot name (e.g., crypto-research-pro): " BOT_NAME
BOT_NAME="${BOT_NAME:-my-openclaw-bot}"

# 2. Description
read -p "📝 Short description: " BOT_DESC
...
# 5. API Key
read -p "🔑 $LLM_PROVIDER API Key: " LLM_API_KEY
...
if [ ! -f wallet.pem ]; then
  ...
  echo "   ✅ wallet.pem created"
else
  echo "   ℹ️  wallet.pem already exists, skipping"
fi
...
EOF
echo "   ✅ agent.config.json created"
